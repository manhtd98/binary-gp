...
if __name__ == "__main__":
    df = pd.read_csv("../data/breast-cancer-wisconsin.data", header=None)#.astype(float)
    df = df.replace("?", 0)
    target_name = df.columns[-1]
    # print(df.iloc[0], df[target_name].unique())
    X_train = df.values[:, 1:10].astype(int)
    y_train = df.values[:, 10].astype(int).reshape(-1, 1)/2-1
    num_attr = X_train.shape[1]
    sample = int(X_train.shape[0]*0.21)
    # print(y_train[0])
